...
First, we'll load the images from the queue you created.
While training, we’ll apply preprocessing to the image.
The preprocessing will add random transformations to the image, creating more images to train on.
...
After these embeddings are created, we’ll use them as feature inputs into
a scikit-learn’s SVM classifier to train on each identity.
...
def main(input_directory, model_path, classifier_output_path,
        batch_size, num_threads, num_epochs, min_images_per_labels,
        split_ratio, is_train=True):
    """
    ...
    with tensorflow.compat.v1.Session(config=tensorflow.compat.v1.ConfigProto(log_device_placement=False)) as sess:
        ...
        init_op = tensorflow.compat.v1.group(tensorflow.compat.v1.global_variables_initializer(), tensorflow.compat.v1.local_variables_initializer())
        sess.run(init_op)
